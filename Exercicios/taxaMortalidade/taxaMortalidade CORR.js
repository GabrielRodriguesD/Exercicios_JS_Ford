...
function calculaNatalidadeMortalidade(){
    ...
    do{
        ...
        if (isNaN(numeroHabitantes)) {
            window.alert("Voce precisa digitar um NÚMERO")
        }
    ...
    do{
        opcao = window.prompt(`Digite a opção desejada:
    ...
    switch (opcao) {
        case "N":
            ...
            const numeroNascimentos = Number(window.prompt("Digite o numero de Nascimentos"));
            window.alert(`A taxa de Mortalidade é ${numeroNascimentos * 1000 / numeroHabitantes}`);
            break;
        case "M":
            const numeroObitos = Number(window.prompt("Digite o numero de Habitantes"));
